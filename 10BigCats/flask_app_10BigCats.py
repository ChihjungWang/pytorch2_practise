...
def recognition(img_path):
    ...
    probability_2 = str(round(probability*100,3))+' %'
    class_probs = list(zip(class_names, probabilities))
    class_probs.sort(key=lambda x:x[1], reverse = True)
    return class_probs, predicted_class, probability_2
...
def predict(delete_file=True):
    ...
    class_probs, predicted_class, probability = recognition(img_path)
    #  在日誌中記錄辨識結果
    app.logger.info("Result : {}".format(str(predicted_class)))
    # 辨識結束之後，可以選擇刪除臨時暫時的圖片
    if delete_file:
        os.remove(img_path)
    return render_template('10BigCats_predict.html', class_probs=class_probs, predicted_class=predicted_class, probability=probability)

@app.route('/')
def home():
    return render_template("10BigCats_home.html")
